...
module PlainRouter::OpenAPI
  class SetDestinationTestMode200ResponseDestination < ApiModelBase
    attr_accessor :id
    ...
    def self.openapi_types
      {
        ...
        :'config' => :'GetEvent200ResponseEventConsent',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PlainRouter::OpenAPI::SetDestinationTestMode200ResponseDestination` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PlainRouter::OpenAPI::SetDestinationTestMode200ResponseDestination`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
